...
require_free_space() {
  required_kb="$1"
  target_dir="${2:-/overlay}"
  available_kb="$(df -Pk "$target_dir" 2>/dev/null | awk 'END {print $4}')"
...
EOF
    ...
    if [ -f /etc/init.d/AdGuardHome ]; then
      [ "$adguard_work" = "/tmp/AdGuardHomeWork" ] && sed -i '/start_service() {/a\    mkdir -p /tmp/AdGuardHomeWork && chmod 700 /tmp/AdGuardHomeWork' /etc/init.d/AdGuardHome
      sed -i '/start_service() {/a\    iptables -t nat -C prerouting_rule -i br-lan -p udp --dport domain -j REDIRECT --to-ports 5353 2>/dev/null || iptables -t nat -A prerouting_rule -i br-lan -p udp --dport domain -j REDIRECT --to-ports 5353\n    iptables -t nat -C prerouting_rule -i br-lan -p tcp --dport domain -j REDIRECT --to-ports 5353 2>/dev/null || iptables -t nat -A prerouting_rule -i br-lan -p tcp --dport domain -j REDIRECT --to-ports 5353' /etc/init.d/AdGuardHome
      sed -i '/stop_service() {/a\    iptables -t nat -D prerouting_rule -i br-lan -p udp --dport domain -j REDIRECT --to-ports 5353 2>/dev/null\n    iptables -t nat -D prerouting_rule -i br-lan -p tcp --dport domain -j REDIRECT --to-ports 5353 2>/dev/null' /etc/init.d/AdGuardHome
    fi
    if ! grep -q "AdGuardHome" /etc/firewall.user 2>/dev/null; then
      cat >> /etc/firewall.user << 'FWEOF'

# AdGuard Home DNS redirect
if pgrep -f "/opt/AdGuardHome/AdGuardHome" >/dev/null 2>&1; then
    iptables -t nat -C prerouting_rule -i br-lan -p udp --dport domain -j REDIRECT --to-ports 5353 2>/dev/null || iptables -t nat -A prerouting_rule -i br-lan -p udp --dport domain -j REDIRECT --to-ports 5353
    iptables -t nat -C prerouting_rule -i br-lan -p tcp --dport domain -j REDIRECT --to-ports 5353 2>/dev/null || iptables -t nat -A prerouting_rule -i br-lan -p tcp --dport domain -j REDIRECT --to-ports 5353
fi
FWEOF
    fi
...
app_wireguard() {
  ...
  wireguard_install_tun() {
    ...
    firmware_version="$(uci -q get version.@version[0].version)"
    if [ "$cpu_type" = "armv7l" ] && [ "$(uname -r)" = "4.1.52" ] &&
      { [ "$firmware_version" = "19.4.0866-3401052" ] || echo "$firmware_version" | grep -q '^19\.4'; }; then
